...
const EJ_PRICE = 3000;
const CL_PRICE = 2450;
const COMPLEX_DISCOUNT = 0.15; // скидка на бандл относительно покупки ЭЖ + ЧЛ по отдельности

const BASE = {
  ej: { key: "ej", label: "МОДУЛЬ «ЭЛЕКТРОННЫЕ ЖУРНАЛЫ»", price: EJ_PRICE },
  cl: { key: "cl", label: "МОДУЛЬ «ЧЕК-ЛИСТЫ КОНТРОЛЯ»", price: CL_PRICE },
  complex: {
    key: "complex",
    label: "ЭКОСИСТЕМА «КОМПЛЕКС» — полный контроль: журналы + индивидуальные чек-листы для любых процессов",
    // база = (ЭЖ + ЧЛ) со скидкой 15% — так бандл гарантированно дешевле суммы отдельных модулей на любом сроке и числе точек
    price: (EJ_PRICE + CL_PRICE) * (1 - COMPLEX_DISCOUNT),
  },
};
...
async function downloadDocx() {
  ...
  children.push(
    ...
    boxWrap([
      ...
      bonusLine(
        "2. СОПРОВОЖДЕНИЕ.",
        "Техническая поддержка на всех этапах масштабирования. Обучение ключевых сотрудников и администраторов сети — текстовые и видео инструкции."
      ),
    ]),
    new Paragraph({ text: "" }),
    boxWrap([
